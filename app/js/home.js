class HomeModule {
      ...
      try {
        ...
        if (HomeModule.lastInstance === me) {
          if (this.common.network) {
            var frContractAddress =
              document.getElementById("frContractAddress");
            if (frContractAddress) {
              frContractAddress.value = this.common.network.rewardToken.address;
            }
            var fairContractAddress = document.getElementById(
              "fairContractAddress"
            );
            if (fairContractAddress) {
              fairContractAddress.value =
                this.common.network.stakedToken.address;
            }
  ...
  async updateGlobalStats(isAutoRefresh) {
    ...
    try {
      ...
      // 获取FAIR正在解押数量
      const totalUnstakingResult =
        await this.contractInteraction.totalUnstaking();
      ...
      const fairTotalUnstakingElement =
        document.getElementById("fairTotalUnstaking");

      if (frTotalSupplyElement) {
        frTotalSupplyElement.value = totalSupplyResult.success
          ? totalSupplyResult.data
          : "0.00";
      }

      if (fairTotalStakedElement) {
        fairTotalStakedElement.value = totalStakedResult.success
          ? totalStakedResult.data
          : "0.00";
      }

      if (fairTotalUnstakingElement) {
        fairTotalUnstakingElement.value = totalUnstakingResult.success
          ? totalUnstakingResult.data
          : "0.00";
      }
    } catch (error) {
