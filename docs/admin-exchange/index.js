  ...
  async fetchContractTokenBalance(token) {
    ...
    try {
      ...
      switch (token.type) {
        ...
        case 3: // TRC10
          {
            // 对于TRC10，tokenAddress实际上是tokenID
            const tokenId = parseInt(token.address);
            const balance =
              await this.admin.contractInteraction.getTRC10Balance(
                contractAddress,
                tokenId
              );
            balanceNum =
  ...
  async fetchTokenExchangeInfo(token) {
    try {
      ...
      const result = await fairStakeContract
        .tokenExchangeInfo(token.tokenAddress || token.address)
        .call();
